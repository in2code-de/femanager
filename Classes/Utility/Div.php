...
class Div
{
     ...
     * logRepository
     *
     * @var \In2code\Femanager\Domain\Repository\LogRepository
     * @inject
     */
    ...
    public function overrideUserGroup($object, $settings, $controllerName = 'new')
    {
        ...
        $object->removeAllUsergroups();
        $usergroupUids = GeneralUtility::trimExplode(',', $settings[$controllerName]['overrideUserGroup'], 1);
    ...
    public static function checkExtension($filename)
    {
        ...
        $settings = $GLOBALS['TSFE']->tmpl->setup['plugin.']['tx_femanager.']['settings.'];
    ...
    public function log($title, $state, User $user)
    {
        ...
        /* @var $log Log */
        $log = $this->objectManager->get('In2code\\Femanager\\Domain\\Model\\Log');
        $log->setTitle($title);
        $log->setState($state);
        $log->setUser($user);
    ...
    public static function sendPost($user, $config, $contentObject)
    {
        ...
        $curl = array(
            ...
            'data' => $contentObject->cObjGetSingle(
                $config['new.']['sendPost.']['data'],
                $config['new.']['sendPost.']['data.']
            ),
        ...
        curl_setopt($curlObject, CURLOPT_URL, $curl['url']);
        curl_setopt($curlObject, CURLOPT_POST, 1);
        curl_setopt($curlObject, CURLOPT_POSTFIELDS, $curl['data']);
    ...
    public static function storeInDatabasePreflight($user, $config, $contentObject, $objectManager)
    {
        ...
        foreach ((array) $config['new.']['storeInDatabase.'] as $table => $storeSettings) {
            ...
            /**
             * @var $storeInDatabase \In2code\Femanager\Utility\StoreInDatabase
             */
            $storeInDatabase = $objectManager->get('In2code\\Femanager\\Utility\\StoreInDatabase');
