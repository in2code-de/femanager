...
class UserController extends AbstractFrontendController
{
    ...
    public function loginAsAction(User $user, int $redirectPid = 1): ResponseInterface
    {
        ...
        $redirectUri = $this->uriBuilder
            ...
            ->build();
        $uriParameters = UserUtility::generateLoginAsParametersWithHmac($user->getUid(), $redirectUri);
        $site = GeneralUtility::makeInstance(SiteFinder::class)?->getSiteByPageId($redirectPid);
        $uri = $site->getRouter()->generateUri($redirectPid, $uriParameters);
